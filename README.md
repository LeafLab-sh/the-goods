...
| `itemDenyList` | *(empty)* | Item IDs (e.g. `minecraft:diamond`) or tag references (e.g. `#minecraft:tools`) excluded from trading. Ignored if `itemAllowList` is non-empty. Denylisting an item that already has stock still lets that stock be bought down to 0 — it only blocks *new* deposits from that point on |
